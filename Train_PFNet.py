...
dset = shapenet_part_loader.PartDataset( root='../PF-Net-Point-Fractal-Network - 복사본/dataset/shapenetcore_partanno_segmentation_benchmark_v0/',classification=True, class_choice=None, npoints=opt.pnum, split='train')
assert dset
dataloader = torch.utils.data.DataLoader(dset, batch_size=opt.batchSize,
                                         shuffle=True,num_workers = int(opt.workers))


test_dset = shapenet_part_loader.PartDataset( root='../PF-Net-Point-Fractal-Network - 복사본/dataset/shapenetcore_partanno_segmentation_benchmark_v0/',classification=True, class_choice=None, npoints=opt.pnum, split='test')
...
optimG = torch.optim.Adam(itertools.chain(paramsG_a2b, paramsG_b2a), lr=0.0001, betas=(0.9,0.999), eps=1e-05, weight_decay=opt.weight_decay)
optimD = torch.optim.Adam(itertools.chain(paramsD_a, paramsD_b), lr=0.0001, betas=(0.9, 0.999), eps=1e-05, weight_decay=opt.weight_decay)
schedulerG = torch.optim.lr_scheduler.StepLR(optimG, step_size=40, gamma=0.2)
shcedulerD = torch.optim.lr_scheduler.StepLR(optimD, step_size=40, gamma=0.2)
#
    ...
    if opt.D_choose == 1:
        for epoch in range(resume_epoch,opt.niter):
            ...
            for i, data in enumerate(dataloader, 0):
                ...
                if opt.cropmethod == 'random_center':
                    ...
                    for m in range(batch_size):
                        ...
                        for sp in range(opt.crop_point_num_b2a): # YH
                            input_cropped1_b.data[m,0,distance_order[sp][0]] = real_point[m,0,distance_order[512+sp][0]] # YH
                ...
                input_cropped3_b_idx = utils.farthest_point_sample(input_cropped1_b,opt.point_scales_list[2],RAN=False)
                ...
                output_a = netD_a(real_center)
                errD_real_a = criterion(output_a, label)
                errD_real_a.backward()
                # netD_b
                netD_b.zero_grad()
                real_center_b = torch.unsqueeze(real_center_b,1)
                output_b = netD_b(real_center_b)
                errD_real_b = criterion(output_b, label)
                errD_real_b.backward()
                # netG_a2b
                fake_center1_a, fake_center2_a, fake_a = netG_a2b(input_cropped)
                # second_input_a2b_b2a = [fake_center1_a, fake_center2_a, fake_a] # 두번째 반복을 위한 저장
                second_input_a2b_b2a = fake_a # 두번째 반복을 위한 저장
                fake_a = torch.unsqueeze(fake_a,1)
                label.data.fill_(fake_label)
                output_a = netD_a(fake_a.detach())
                errD_fake_a = criterion(output_a, label)
                errD_fake_a.backward()
                errD_a = errD_real_a + errD_fake_a
                # netG_b2a
                fake_center1_b, fake_center2_b, fake_b = netG_b2a(input_cropped_b)
                # second_input_b2a_a2b = [fake_center1_b, fake_center2_b, fake_b] # 두번째 반복을 위한 저장
                second_input_b2a_a2b = fake_b # 두번째 반복을 위한 저장
                fake_b = torch.unsqueeze(fake_b,1)
                label.data.fill_(fake_label)
                output_b = netD_b(fake_b.detach())
                errD_fake_b = criterion(output_b, label)
                errD_fake_b.backward()
                ...
                output_a = netD_a(fake_a)
                errG_D_a = criterion(output_a, label)
                errG_l2_a = 0
                ...
                errG_a = (1-opt.wtl2) * errG_D_a + opt.wtl2 * errG_l2_a
                errG_a.backward()
                # netG_b2a
                netG_b2a.zero_grad()
                label.data.fill_(real_label)
                output_b = netD_b(fake_b)
                errG_D_b = criterion(output_b, label)
                errG_l2_b = 0
                ...
                errG_b = (1-opt.wtl2) * errG_D_b + opt.wtl2 * errG_l2_b
                errG_b.backward()
                optimG.step()
                # print
                print('/*########################################*/')
                print('[%d/%d][%d/%d] Loss_D_a: %.4f Loss_G_a: %.4f / %.4f / %.4f/ %.4f'
                      % (epoch, opt.niter, i, len(dataloader),
                         errD_a.data, errG_D_a.data, errG_l2_a, errG_a, CD_LOSS_a))
                print('[%d/%d][%d/%d] Loss_D_b: %.4f Loss_G_b: %.4f / %.4f / %.4f/ %.4f'
                      % (epoch, opt.niter, i, len(dataloader),
                         errD_b.data, errG_D_b.data, errG_l2_b, errG_b, CD_LOSS_b))
                f = open('loss_PFNet.txt', 'a')
                f.write('\n' + '[%d/%d][%d/%d] Loss_D_a: %.4f Loss_G_a: %.4f / %.4f / %.4f /%.4f'
                        % (epoch, opt.niter, i, len(dataloader),
                           errD_a.data, errG_D_a.data, errG_l2_a, errG_a, CD_LOSS_a))
                f.write('\n' + '[%d/%d][%d/%d] Loss_D_b: %.4f Loss_G_b: %.4f / %.4f / %.4f /%.4f'
                        % (epoch, opt.niter, i, len(dataloader),
                           errD_b.data, errG_D_b.data, errG_l2_b, errG_b, CD_LOSS_b))
                #
                ...
                #*****중요*****#
                #second에서는 a2b_b2a가 real_center_b세트를 GT로 갖게되고, b2a_a2b가 real_center세트를 GT로 갖게 되기 때문에 따로 준비할 필요가없음#
                #*****중요*****#
                # input_cropped_a2b_b2a / input_cropped_b2a_a2b만 IFPS적용해서 다운샘플링
                input_cropped_a2b_b2a_2_idx = utils.farthest_point_sample(input_cropped_a2b_b2a, opt.point_scales_list[1],RAN=True)
                input_cropped_a2b_b2a_2     = utils.index_points(input_cropped_a2b_b2a, input_cropped_a2b_b2a_2_idx)
                input_cropped_a2b_b2a_3_idx = utils.farthest_point_sample(input_cropped_a2b_b2a, opt.point_scales_list[2],RAN=False)
                ...
                input_cropped_b2a_a2b_2_idx = utils.farthest_point_sample(input_cropped_b2a_a2b, opt.point_scales_list[1],RAN=True)
                input_cropped_b2a_a2b_2     = utils.index_points(input_cropped_b2a_a2b, input_cropped_b2a_a2b_2_idx)
                input_cropped_b2a_a2b_3_idx = utils.farthest_point_sample(input_cropped_b2a_a2b, opt.point_scales_list[2],RAN=False)
                ...
                ###########################
                # netG_b2a
                fake_center_a2b_b2a_1, fake_center_a2b_b2a_2, fake_center_a2b_b2a = netG_b2a(input_cropped_second_a2b_b2a)
                fake_center_a2b_b2a = torch.unsqueeze(fake_center_a2b_b2a,1)
                output_a2b_b2a = netD_b(fake_center_a2b_b2a.detach())
                errD_fake_a2b_b2a = criterion(output_a2b_b2a, label)
                errD_fake_a2b_b2a.backward()
                # errD_a2b_b2a = errD_real_a2b_b2a + errD_fake_a2b_b2a # 이거 쓸지안쓸지 고민좀
                # netG_a2b
                fake_center_b2a_a2b_1, fake_center_b2a_a2b_2, fake_center_b2a_a2b = netG_a2b(input_cropped_second_b2a_a2b)
                fake_center_b2a_a2b = torch.unsqueeze(fake_center_b2a_a2b,1)
                label.data.fill_(fake_label)
                output_b2a_a2b = netD_a(fake_center_b2a_a2b.detach())
                errD_fake_b2a_a2b = criterion(output_b2a_a2b, label)
                errD_fake_b2a_a2b.backward()
                # errD_b2a_a2b = errD_real_b2a_a2b + errD_fake_b2a_a2b # 이거 쓸지안쓸지 고민좀
                optimD.step()

                ############################
                # (6) YH: Update G Network for second
                ###########################
                # netG_b2a
                # netG_b2a.zero_grad() # 이거 안써야할거같음
                label.data.fill_(real_label)
                output_a2b_b2a = netD_b(fake_center_a2b_b2a)
                errG_D_a2b_b2a = criterion(output_a2b_b2a, label)
                errG_l2_a2b_b2a = 0
                CD_LOSS_a2b_b2a = criterion_PointLoss(torch.squeeze(fake_center_a2b_b2a,1),torch.squeeze(real_center_b,1))
                errG_l2_a2b_b2a = criterion_PointLoss(torch.squeeze(fake_center_a2b_b2a,1),torch.squeeze(real_center_b,1))\
                ...
                errG_a2b_b2a = (1-opt.wtl2) * errG_D_a2b_b2a + opt.wtl2 * errG_l2_a2b_b2a
                errG_a2b_b2a.backward()
                # netG_a2b
                # netG_a2b.zero_grad() # 이거 안써야할거같음
                label.data.fill_(real_label)
                output_b2a_a2b = netD_a(fake_center_b2a_a2b)
                errG_D_b2a_a2b = criterion(output_b2a_a2b, label)
                errG_l2_b2a_a2b = 0
                CD_LOSS_b2a_a2b = criterion_PointLoss(torch.squeeze(fake_center_b2a_a2b,1),torch.squeeze(real_center,1))
                errG_l2_b2a_a2b = criterion_PointLoss(torch.squeeze(fake_center_b2a_a2b,1),torch.squeeze(real_center,1))\
                ...
                errG_b2a_a2b = (1-opt.wtl2) * errG_D_b2a_a2b + opt.wtl2 * errG_l2_b2a_a2b
                errG_b2a_a2b.backward()
                optimG.step()
                # print
                print('/*########################################*/')
                print('[%d/%d][%d/%d] Loss_D_a2b_b2a: %.4f Loss_G_a2b_b2a: %.4f / %.4f / %.4f/ %.4f'
                      % (epoch, opt.niter, i, len(dataloader),
                         errD_fake_a2b_b2a.data, errG_D_a2b_b2a.data, errG_l2_a2b_b2a, errG_a2b_b2a, CD_LOSS_a2b_b2a))
                print('[%d/%d][%d/%d] Loss_D_b2a_a2b: %.4f Loss_G_b2a_a2b: %.4f / %.4f / %.4f/ %.4f'
                      % (epoch, opt.niter, i, len(dataloader),
                ...
                if i % 10 ==0:
                    ...
                    for i, data in enumerate(test_dataloader, 0):
                        ...
                        f.write('\n' + 'test result: CD_loss:%.4f CD_loss_b:%.4f SUM:%.4f' % (CD_loss, CD_loss_b, CD_loss + CD_loss_b)) # YH
                        break
                f.close()
            schedulerD.step()
            schedulerG.step()
